...
(function() {
  var Collection, WebSQLDb, _, async, compileSort, doNothing, processFind, utils;
      ...
      return this.db.readTransaction((function(_this) {
        return function(tx) {
          return tx.executeSql("SELECT * FROM docs WHERE col = ?", [_this.name], function(tx, results) {
            var docs, i, j, ref, row;
            docs = [];
            for (i = j = 0, ref = results.rows.length; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
      ...
      return this.db.transaction((function(_this) {
        return function(tx) {
          ...
          return async.eachSeries(ids, function(id, callback) {
            ...
            var base, id, item, j, len, results1;
            results1 = [];
            for (j = 0, len = items.length; j < len; j++) {
              ...
              results1.push(tx.executeSql("INSERT OR REPLACE INTO docs (col, id, state, doc, base) VALUES (?, ?, ?, ?, ?)", [_this.name, item.doc._id, "upserted", JSON.stringify(item.doc), JSON.stringify(base)], doNothing, (function(tx, err) {
                return error(err);
              })));
            }
            return results1;
